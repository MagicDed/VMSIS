...
int main()
{
    ...
    do
    {
        ...
        for(int i = 0; i < row; i++)                                            // Цикл нахождения минмакс и минмакс
        {
            for(int j = 0; j < col; j++)
            {
                if(mas[i * col + j] < minmax)                                           // Ищем самое минимальное
                {
                    minmax = mas[i * col + j];                                              // Если меньше то оно новое минимальное
                }
                if(mas[i * col + j] > maxmin)                                           // Ищем самое максимальное
                {
                    maxmin = mas[i * col + j];                                              // Если меньше то оно новое максимальное
                }
            }
        }
        for(int i = 0; i < row; i++)                                            // Цикл нахождения минмакс и минмакс 
        {
            for(int j = 0; j < col; j++)
            {
                if(mas[i * col + j] < 0 && mas[i * col + j] > minmax)                   // Если число меньше минмакс и меньше нуля
                {
                    co1 = i;                                                                // Запоминаем ее координаты
                    co2 = j;                                                                // Запоминаем ее координаты
                }
                if(mas[i * col + j] >= 0 && mas[i * col + j] < maxmin)                  // Если число меньше максмин и меньше нуля
                {
        ...
        t = mas[co1 * col + co2];                                               // Минмакс запоминается
        mas[co1 * col + co2] = mas[co3 * col + co4];                            // Вместо него пишем максмин
        mas[co3 * col + co4] = t;                                               // Вставляем минмакс вместо максмин
        for(int i = 0; i < row; i++)                                            // Цикл для вывода матрицы
        {
            for(int j = 0; j < col; j++)
            {
                printf("%d\t", mas[i * col + j]);                                       // Вывод матрицы
            }
            printf("\n");
        }
        printf("Maxmin: %d\nMinmax: %d\n", mas[co3 * col + co4], mas[co1 * col + co2]);           // Вывод минмакс и максмин 
